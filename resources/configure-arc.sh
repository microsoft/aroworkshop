...
az connectedk8s connect --name ${CLUSTER_NAME}-arc --resource-group $RESOURCE_GROUP #--no-wait
...
az k8s-extension create -g $RESOURCE_GROUP -c $CLUSTER_NAME-arc -t connectedClusters --extension-type Microsoft.AzureKeyVaultSecretsProvider --name akvsecretsprovider --no-wait
